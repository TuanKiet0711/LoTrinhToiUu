...
// luôn dùng [lat, lng] đúng chuẩn Leaflet
...
// ==== SEARCH BOX (chỉ tìm trong marker đã render) ====
...
if (searchBox) {
    searchBox.addEventListener("keyup", (e) => {
        ...
        // lấy tất cả marker từ map
        let found = null;
        map.eachLayer(layer => {
            if (layer instanceof L.Marker && layer._popup) {
                const name = (layer._popup.getContent() || "").toLowerCase();
        ...
        if (found) {
            ...
            alert("Không tìm thấy điểm nào phù hợp!");
        }
    });
}
